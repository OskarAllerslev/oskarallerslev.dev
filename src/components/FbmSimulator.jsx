			...
			<div className="mt-4 space-y-4 text-sm text-zinc-400">
				...
				<div>
					...
					<p>
						...
						<InlineMath math={String.raw`S_t`} /> and its variance <InlineMath math={String.raw`V_t`} />:
					</p>
					<BlockMath math={String.raw`dS_t = S_t \sqrt{V_t} dW_t`} />
					<BlockMath math={String.raw`V_t = V_0 \exp\left( \eta B_H(t) - \frac{1}{2} \eta^2 t^{2H} \right)`} />
				</div>
				<div>
					<h6 className="font-semibold text-zinc-300">fBM Simulation for H &lt; 0.5</h6>
					<p>
						Simulating the fractional Brownian motion <InlineMath math={String.raw`B_H(t)`} /> for a Hurst parameter{' '}
						<InlineMath math={String.raw`H < 0.5`} /> requires careful consideration of its auto-covariance structure. The
						covariance matrix <InlineMath math={String.raw`\Sigma`} /> for a time grid{' '}
						<InlineMath math={String.raw`t_1, t_2, \dots, t_n`} /> is given by:
					</p>
					<BlockMath
						math={
							String.raw`\Sigma_{ij} = \mathbb{E}[B_H(t_i)B_H(t_j)] = \frac{1}{2}(t_i^{2H} + t_j^{2H} - |t_i - t_j|^{2H})`
						}
					/>
					<p className="mt-2">
						To generate valid paths, the engine performs an exact Cholesky decomposition of this dense
						covariance matrix, <InlineMath math={String.raw`\Sigma = LL^T`} />. Correlated random variates are then
						generated by transforming a vector of independent standard normal variables{' '}
						<InlineMath math={String.raw`Z`} /> via <InlineMath math={String.raw`X = LZ`} />. This method is computationally
